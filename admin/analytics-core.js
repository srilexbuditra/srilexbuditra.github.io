(() => {
  ...
  const analyticsRoot =
    document.querySelector(
      'main.content > [data-sb-view="analytics"]'
    );
...
const refreshBtn = analyticsRoot.querySelector("#refreshBtn");
const exportBtn = analyticsRoot.querySelector("#exportBtn");
const statusBox = analyticsRoot.querySelector("#status");
const dashboard = analyticsRoot.querySelector("#dashboard");
const updatedAt = analyticsRoot.querySelector("#updatedAt");
const startDateInput = analyticsRoot.querySelector("#startDate");
const endDateInput = analyticsRoot.querySelector("#endDate");
const applyFilterBtn = analyticsRoot.querySelector("#applyFilterBtn");
const resetFilterBtn = analyticsRoot.querySelector("#resetFilterBtn");
const periodLabel = analyticsRoot.querySelector("#periodLabel");
...
function setPreset(range) {
  ...
  analyticsRoot.querySelectorAll(".preset-btn").forEach(b=>b.classList.toggle("active",b.dataset.range===range));
  updatePeriodLabel();
}
function statsUrl() {
...
function renderRecentVisits(rows, onlineDetails = []) {
  const tbody = analyticsRoot.querySelector("#recentVisits");
  if (!tbody) return;
  tbody.innerHTML = "";
...
function renderTrend(rows, days = 7) {
  const chart = analyticsRoot.querySelector("#trendChart");
  if (!chart) return;
  chart.innerHTML = "";
...
function renderDashboard(data) {
  ...
  const trackingNote = analyticsRoot.querySelector("#trackingNote");
  if (trackingNote) {
...
function renderOnlinePages(items = []) {
  const box = analyticsRoot.querySelector("#onlinePages");
  if (!box) return;
...
analyticsRoot.querySelectorAll(".range-btn").forEach((button) => {
  button.addEventListener("click", () => {
    selectedTrendDays = Number(button.dataset.days || 7);
    analyticsRoot.querySelectorAll(".range-btn").forEach((item) => {
...
analyticsRoot.querySelectorAll(".preset-btn").forEach(button => button.addEventListener("click", () => {
...
applyFilterBtn?.addEventListener("click", () => {
  ...
  analyticsRoot.querySelectorAll(".preset-btn").forEach(b=>b.classList.remove("active"));
...
/* =========================================================
   ...
   ========================================================= */
const v51PresetContainer = analyticsRoot.querySelector(".preset-buttons");
...
applyFilterBtn?.addEventListener(
  ...
  (event) => {
    ...
    analyticsRoot.querySelectorAll(".preset-btn").forEach((btn) => {
