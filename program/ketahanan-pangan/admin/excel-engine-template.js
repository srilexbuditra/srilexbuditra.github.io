...
function styleForRow(r,ws){if(r===0)return 4;if(r===1)return 10;if(r===2)return 16;if(r===4)return 24;if(r>=5)return 33;return 0}
...
async function writeFile(wb,name){
 ...
 const res=await fetch('./excel-template-3sheet-v43.xlsx?v=1',{cache:'no-store'});if(!res.ok)throw new Error(`Template Excel lokal gagal dimuat (HTTP ${res.status}).`);
